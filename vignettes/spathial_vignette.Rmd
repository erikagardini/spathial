...
* __correlations__: This vector contains the Pearson's correlation coefficients between each feature and the path. In the case of multiple paths (see the _neighbor_ parameter, described later in this vignette), a Fisher-integrated correlation coefficient is provided.
...
In the case of multiple paths (see the _neighbor_ parameter, described later in this vignette), an average of the feature correlation coefficient ranks across the perturbed paths is provided.
...
When working with high dimensional data, one of the problems that could occur is the overfitting. This means that the principal path algorithm could find a lot of different features all correlated with the evolution from the starting point to the ending one. 
If you are interested in finding a restricted group of features which are very correlated with the _spathial_ path progression across a dataset, we suggest to use a more complex version of the function ```spathialWay```. It takes as input the parameter ```neighbors > 0``` and perform a refined version of the principal path algorithm.

The following picture graphically explains how this version of the function ```spathialWay``` works when ```neighbors = 2```:

![_Graphical represemtatopm of the perturbed path_](figures/simple_multipath_example.png){width=50%}

It selects the ```neighbors``` nearest data points to the starting and the ending points and computes the principal path algorithm for each combination. This refined procedure allows to compute ```neighbors*neighbors``` paths: one of them is the normal principal path and the others can be considered as perturbation of the first one.

The correlation can now be computed as mean of the Fisher's correlations of each path. Additionally, in the example we provide an average of the feature correlation coefficient ranks across the perturbed paths (```rank_scores```) as extra values to measure feature/path correlation strength.

The following code box shows an example of the ```spathialWay``` function with ```neighbors = 1``` and the same dataset of the previous example. We will load again the RPM-normalized Liver Cancer expression data:
...
head(X)
```
Then, as before, we will select the starting and ending points and run _spathial_ to identify multiple paths between them. This example will take a few seconds, depending on the computer.
```{r results="hide",message="hide"}
#### Choose the starting and the ending points
...
```{r fig.cap="_Rtsne representation of the Liver Cancer dataset with multiple perturbed paths (depicted as lines of different colors)_"}
# Plot the path in 2D using Rtsne
...
We can compare the gene scores obtained with the multiple path approach with those obtained by the single path calculations, stored in ```singlepath_correlations```
```{r fig.cap="_Comparison between feature correlation values in Single Path and Multiple Path_"}
# Compare the results
multipath_correlations<-statistics$correlations
plot(singlepath_correlations,multipath_correlations,pch=20,
     xlab="Gene Correlation Coefficients with Single Path",
     ylab="Gene Correlation Coefficients with Multiple Path"
)
grid()
```
__Warning__: the greater is the value of the parameter ```neighbors > 5```, the slower is the algorithm. Our recommendation is to use ```neighbors <= 5```. 
...
[2]  L. van der Maaten and G. Hinton, “Viualizing data using t-SNE,” Journalof Machine Learning Research, vol. 9, pp. 2579–2605, Nov. 2008.

[3] T.  Cover  and  P.  Hart,  “Nearest  neighbor  pattern  classification,”  IEEETransactions on Information Theory, vol. 13, pp. 21–27, Jan. 1967.
