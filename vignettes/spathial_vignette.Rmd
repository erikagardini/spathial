...
# Quick start
In this section we show the most basic steps to compute the _spathial_ implementation of the principal path [__citation needed__], through a simple 2-dimensional example.
...
spathial_res <- spathial::spathialWay(X, X_labels, boundary_ids, NC, neighbors = 0)
```
In order to visualize the multi-dimensional dataset on two dimensions, we perform a TSNE analysis on it (__CITATION NEEDED__).
...
The correlation can now be computed as mean of the correlations of each path __PLEASE CONSIDER USING THE FISHER WAY AS THE DEFAULT ONE TO AVERAGE CORRELATIONS__. Additionally, in the example we provide the Fisher's method to integrate correlations (```fisher_correlations```) and an average of the feature correlation coefficient ranks across the perturbed paths (```rank_scores```) as extra values to measure feature/path correlation strength.
