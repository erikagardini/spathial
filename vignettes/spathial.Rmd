...
The package _spathial_ has the function ```spathialBoundaryIds``` which takes as input the matrix ```X```, 
...
User can choose the start and the end point with their own strategy, which can be more suitable for their specific problem; in this case, the user should run the function ```spathialBoundaryIds``` with ```mode=3``` specifing the rownames of the samples using the parameters ```from``` and ```to```.
...
The _spathial_ method ```spathialStatistics``` can be used to calculate the Pearson's correlation coefficients and rank the genes according to the correlation scores: 
```{r}
...
Finally, one can use the _spathial_ method to plot the path as follow:
```{r}
spathial::spathialPlot(X, X_labels, boundary_ids, spathial_res, perplexity_value = 30, mask = NULL)
```
